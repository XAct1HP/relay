...
export async function POST(request: Request) {
  try {
    ...
    const report = await processIntegrationInventoryUpsert(
      admin,
      auth.sellerId,
      body?.items
    );
    ...
    console.error("Integration inventory upsert error:", error);
    return NextResponse.json(
      { error: "Failed to process inventory upsert request." },
      { status: 500 }
    );
  }
}
