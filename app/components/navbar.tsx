            ...
            {user ? (
              <>
                <NotificationsNavButton
                  ...
                  initialUnreadConversationCount={unreadConversationCount}
                />
                <Link href={profileHref} className={mobileChipClassName()}>
                  Profile
                </Link>
              </>
            ) : (
        ...
        <div className="md:hidden">
          {user ? (
            <div className="grid grid-cols-3 gap-2">
              <Link href="/marketplace" className={mobileChipClassName()}>
                Market
              </Link>
              <Link href="/messages" className={mobileChipClassName()}>
                Messages
              </Link>
              <Link href="/sell" className={mobileChipClassName()}>
                Sell
              </Link>
              <Link href="/orders" className={mobileChipClassName()}>
                Orders
              </Link>
              <Link href="/my-listings" className={mobileChipClassName()}>
                Listings
              </Link>
              <Link href="/dashboard" className={mobileChipClassName()}>
                {isAdmin ? "Admin" : "Dashboard"}
              </Link>
              <Link href="/profile/studio" className={`${mobileChipClassName()} col-span-2`}>
                Studio
              </Link>
              <div className="flex">{/* keeps logout aligned */}<LogoutButton /></div>
            </div>
          ) : (
            <div className="grid grid-cols-2 gap-2">
              ...
              <Link href="/auth/login" className={mobileChipClassName()}>
                Log In
              </Link>
              <Link
                href="/auth/signup"
